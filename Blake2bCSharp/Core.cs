...
namespace Blake2bCSharp
{
    public sealed partial class Blake2bCore
    {
        ...
        private UInt64[] _h = new UInt64[8]; 
        private UInt64 _counter0;
        private UInt64 _counter1;
        private UInt64 _finalizationFlag0;
        private UInt64 _finalizationFlag1;

        //private const int NumberOfRounds = 12; 
        public const int BlockSizeInBytes = 128; 

        const UInt64 IV0 = 0x6A09E667F3BCC908UL;
        const UInt64 IV1 = 0xBB67AE8584CAA73BUL;
        const UInt64 IV2 = 0x3C6EF372FE94F82BUL;
        const UInt64 IV3 = 0xA54FF53A5F1D36F1UL;
        const UInt64 IV4 = 0x510E527FADE682D1UL;
        const UInt64 IV5 = 0x9B05688C2B3E6C1FUL;
        const UInt64 IV6 = 0x1F83D9ABFB41BD6BUL;
        const UInt64 IV7 = 0x5BE0CD19137E2179UL;
        ...
        internal static UInt64 BytesToUInt64(byte[] buf, int offset)
        {
            return
                ((UInt64)buf[offset + 7] << 7 * 8 |
                ((UInt64)buf[offset + 6] << 6 * 8) |
                ((UInt64)buf[offset + 5] << 5 * 8) |
                ((UInt64)buf[offset + 4] << 4 * 8) |
                ((UInt64)buf[offset + 3] << 3 * 8) |
                ((UInt64)buf[offset + 2] << 2 * 8) |
                ((UInt64)buf[offset + 1] << 1 * 8) |
                ((UInt64)buf[offset]));
        }

        private static void UInt64ToBytes(UInt64 value, byte[] buf, int offset)
        {
        ...
        public void Initialize(UInt64[] salt) //
        {
            if (salt == null)
                throw new ArgumentNullException("salt");
            if (salt.Length != 8)
                throw new ArgumentException("salt length must be 8 qwords");
            _isInitialized = true;
        ...
        public byte[] HashFinal(bool isEndOfLayer)
        {
            ...
            _finalizationFlag0 = UInt64.MaxValue; //
